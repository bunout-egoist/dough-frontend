  ...
  updateSelectedData() {
    ...
    if (Array.isArray(this.state.data)) {
      const selectedData = this.state.data.find(
        (item) => item.completedDate === formattedDate
      );
      ...
      <div className="detail-page page-area">
        <div className="detail-top">
          <div className="detail-top-title">
            <Link to="/dashpage">
              <div className="detail-top-back">
                <img className="img-width" src="/images/back.png" />
              </div>
            </Link>
          ...
          <div className="this-week">
            {[...Array(7)].map((_, i) => {
              ...
              const matchingData = Array.isArray(data)
                ? data.find((item) => item.completedDate === formattedDate)
                : null;
              ...
              return (
                <div
                  key={i}
                  className={`this-week-day ${
                    isToday ? "today" : ""
                  } ${formattedDate} ${
                    dailyCount ? `daily-count-${dailyCount}` : ""
                  }`} // dailyCount를 클래스에 추가
                  onClick={() => this.handleDayClick(i - 3)}
                >
                  <div className="day-txt">{this.getDayWithOffset(i - 3)}</div>
                  <div className="day-date">{formattedDate.slice(-2)}</div>{" "}
                  {/* 날짜의 일만 표시 */}
                </div>
              );
            })}
          ...
          {selectedData ? (
              ...
              <div>
                <ul>
                  {selectedData.questDetails.map((quest, index) => (
                    <li key={index}>
                      <MissionCard
                        pictureImg={quest.imageUrl}
                        missionTitle={quest.activity}
                        missionDetail={quest.description}
                        missionTag={quest.questType}
                      />
                    </li>
                  ))}
                </ul>
            ...
            <div className="no-mission-area">
              <div className="no-mission-img">
                <img
                  src="/images/dashpage/no-mission.png"
                  className="img-width"
                />
              </div>
            </div>
