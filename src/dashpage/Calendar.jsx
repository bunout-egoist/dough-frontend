  ...
  fetchMonthlyData() {
    ...
    fetch(`/api/v1/dashboard/monthly/${yearMonth}`, {
      ...
      .then((data) => {
        ...
        this.state.completedAllQuestsDateCount =
          data.completedAllQuestsDateCount;
      ...
      <div>
        <div className="today-month">
          <div className="display-flex">
            ...
            >
              <path
                d="M2.77773 10.0001L10.1277 17.3501C10.3777 17.6001 10.4986 17.8918 10.4902 18.2251C10.4819 18.5584 10.3527 18.8501 10.1027 19.1001C9.85273 19.3501 9.56107 19.4751 9.22773 19.4751C8.8944 19.4751 8.60273 19.3501 8.35273 19.1001L0.652734 11.4251C0.452734 11.2251 0.302734 11.0001 0.202734 10.7501C0.102734 10.5001 0.0527344 10.2501 0.0527344 10.0001C0.0527344 9.75011 0.102734 9.50011 0.202734 9.25011C0.302734 9.00011 0.452734 8.77511 0.652734 8.57511L8.35273 0.87511C8.60273 0.62511 8.89857 0.504277 9.24023 0.51261C9.5819 0.520944 9.87773 0.65011 10.1277 0.90011C10.3777 1.15011 10.5027 1.44178 10.5027 1.77511C10.5027 2.10844 10.3777 2.40011 10.1277 2.65011L2.77773 10.0001Z"
                fill="#8C96A4"
              />
            </svg>
          </div>
        ...
        <div className="cal-mission">
          {this.state.month.format("M")}월, 모든 미션을 완료한 날은{" "}
          <span className="bold-txt orange-txt">
            {this.state.completedAllQuestsDateCount}번
          </span>
          이에요!
          <br />
          조금만 더 힘내세요!
...
class Week extends Component {
  render() {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      let missionCount =
        contents && contents[date.format("YYYY-MM-DD")]
          ? contents[date.format("YYYY-MM-DD")].length
          : 0;
...
class Day extends Component {
  render() {
    ...
    const matchingDetail = countDetails.find(
      (detail) => detail.completedDate === dateToMatch
    );
