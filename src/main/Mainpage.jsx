...
export default function Mainpage() {
    // Initial state with all mission boxes in order
    const initialMissions = [
        { id: 1, backgroundColor: "#FF7A2F", isChecked: false, missionText:"노래 들으면서 10분 산책하기" },
        { id: 2, backgroundColor: "#8FBCFF", isChecked: false,  missionText:"노래 들으면서 20분 춤추기" },
        { id: 3, backgroundColor: "#FFC13A", isChecked: false, missionText:"노래 들으면서 50분 디제잉하기" },
    ];
    ...
    const handleMissionCheck = (id) => {
        setMissions((prevMissions) => {
            // Find the mission that was clicked
            const updatedMissions = prevMissions.map((mission) =>
                mission.id === id ? { ...mission, isChecked: !mission.isChecked } : mission
            );

            // Separate checked and unchecked missions
            const uncheckedMissions = updatedMissions.filter(m => !m.isChecked);
            const checkedMissions = updatedMissions.filter(m => m.isChecked);

            // Return new order: unchecked missions first, then checked missions
            return [...uncheckedMissions, ...checkedMissions];
        });
    };
